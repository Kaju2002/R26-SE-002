...
const buildActorFromRequest = (req) => {
  ...
  return {
    ...
    actorName: name,
    actorEmail: String(user.email || req.email || ""),
  };
};
...
export const recordAuditLog = async (req, entry) => {
  ...
  if (!baseUrl || !key) {
    console.warn("chat-management: audit log skipped (missing USER_MANAGEMENT_BASE_URL or INTERNAL_SERVICE_KEY)");
    return false;
  }

  const actor = buildActorFromRequest(req);
  if (!actor.actorId) {
    console.warn("chat-management: audit log skipped (missing actor id)");
    return false;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn(
        "chat-management: audit log request failed:",
    ...
    console.warn("chat-management: audit log request error:", error.message);
    return false;
  }
};
