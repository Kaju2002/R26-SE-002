...
app.get("/", (req, res) => {
  res.json({
    ...
    endpoints: {
      ...
      updateStatus:
        "PATCH /api/chat/conversations/:conversationId/status (Bearer token required, body: { status: \"active\" | \"archived\" | \"blocked\" })",
      updateSaved:
        "PATCH /api/chat/conversations/:conversationId/saved (Bearer token required, body: { saved: boolean })",
