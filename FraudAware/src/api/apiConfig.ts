...
/**
 * Set `EXPO_PUBLIC_CHAT_MANAGEMENT_API_BASE_URL` in `FraudAware/.env`
 * (e.g. `http://192.168.1.250:5003`, no trailing slash). Restart Expo after changing.
 */
export function getChatManagementBaseUrl(): string {
  return (
    (process.env.EXPO_PUBLIC_CHAT_MANAGEMENT_API_BASE_URL ?? '').trim().replace(/\/$/, '') ||
    'http://127.0.0.1:5003'
  );
}
